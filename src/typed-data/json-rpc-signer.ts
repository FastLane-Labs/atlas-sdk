...
export async function signTypedData_signer(
  ...
  const populated = await TypedDataEncoder.resolveNames(
    ...
    value,
    async (name: string) => {
      const resolvedName = await signer.provider.resolveName(name);
      return resolvedName || ""; // Return an empty string if the resolved name is null
    }
  );
